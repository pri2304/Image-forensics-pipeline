...
# --- 1. CONFIGURATION ---
OLD_INPUT_CSV = "Datasets/gbt_training_features_flattened.csv"
NEW_INPUT_CSV = "/home/pri/PycharmProjects/videotojpeg/custom_forensic_dataset.csv"

# Temp Files (We will merge these at the end)
TEMP_OLD_UPDATED = "temp_updated_old.csv"
TEMP_NEW_EXTRACTED = "temp_new_features.csv"
...
def process_extract_new(row):
    ...
    features = {
        "path": path,
        "label": row['label'],
        "label_str": row.get('label_str', 'Unknown'),
...
# --- 5. MAIN ---
def main():
    print("--- ROBUST FEATURE MERGER ---")

    if not init_resources():
        return

    # === PHASE 1: UPDATE EXISTING CSV ===
    print(f"\n🔄 Phase 1: Updating Old Features ({OLD_INPUT_CSV})")

    processed_paths = set()
    updated_rows = []

    if os.path.exists(OLD_INPUT_CSV):
        df_old = pd.read_csv(OLD_INPUT_CSV)
        print(f"   -> Loaded {len(df_old)} rows.")

        # Convert to list of dicts for safety
        rows = df_old.to_dict('records')

        for i, row in enumerate(tqdm(rows, desc="Updating Old")):
            # Update the row (returns original if fails)
            new_row = process_update_row(row)

            updated_rows.append(new_row)
            processed_paths.add(new_row['path'])

            # Incremental Save every 50 rows
            if len(updated_rows) % 50 == 0:
                pd.DataFrame(updated_rows).to_csv(TEMP_OLD_UPDATED, index=False)

        # Final Phase 1 Save
        pd.DataFrame(updated_rows).to_csv(TEMP_OLD_UPDATED, index=False)
        print(f"✅ Phase 1 Complete. Saved {len(updated_rows)} rows to {TEMP_OLD_UPDATED}")
    else:
        print("⚠️ Old CSV not found. Skipping Phase 1.")

    # === PHASE 2: EXTRACT NEW FEATURES ===
    print(f"\n🆕 Phase 2: Extracting New Features ({NEW_INPUT_CSV})")

    extracted_rows = []

    if os.path.exists(NEW_INPUT_CSV):
        df_new = pd.read_csv(NEW_INPUT_CSV)

        # Filter duplicates
        new_targets = [r for _, r in df_new.iterrows() if r['path'] not in processed_paths]
        print(f"   -> Found {len(new_targets)} unique new images to process.")

        for i, row in enumerate(tqdm(new_targets, desc="Extracting New")):
            ...
            if res:
                ...
                if len(extracted_rows) % 50 == 0:
                    pd.DataFrame(extracted_rows).to_csv(TEMP_NEW_EXTRACTED, index=False)

        if extracted_rows:
            pd.DataFrame(extracted_rows).to_csv(TEMP_NEW_EXTRACTED, index=False)
            print(f"✅ Phase 2 Complete. Saved {len(extracted_rows)} rows to {TEMP_NEW_EXTRACTED}")
    else:
        print("⚠️ New CSV not found. Skipping Phase 2.")

    # === PHASE 3: MERGE ===
    print(f"\n🔗 Phase 3: Merging Files...")

    dfs = []
    if os.path.exists(TEMP_OLD_UPDATED):
        dfs.append(pd.read_csv(TEMP_OLD_UPDATED))
    if os.path.exists(TEMP_NEW_EXTRACTED):
        dfs.append(pd.read_csv(TEMP_NEW_EXTRACTED))

    if dfs:
        final_df = pd.concat(dfs, ignore_index=True)

        # Sort Columns
        front_cols = ['path', 'label', 'label_str', 'tag1', 'cnn_score', 'nat_score']
        cols = [c for c in front_cols if c in final_df.columns]
        cols += [c for c in final_df.columns if c not in cols]
        final_df = final_df[cols]

        final_df.to_csv(FINAL_MERGED_CSV, index=False)
        print(f"✅ SUCCESS! Final file saved: {FINAL_MERGED_CSV}")
